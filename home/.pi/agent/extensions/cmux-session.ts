// Local cmux integration for Pi.
// Uses one shared workspace status while retaining Feed telemetry, notifications, and resume bindings.
// This file intentionally omits cmux's managed-extension marker so automatic hook refreshes cannot overwrite it.
...
function setSharedStatus(ctx: ExtensionContext, value: "Running" | "Idle"): boolean {
  if (process.env.CMUX_PI_HOOKS_DISABLED === "1" || !process.env.CMUX_SURFACE_ID) return true;
  ...
  const args = [
    ...
    value,
    "--icon",
    value === "Running" ? "bolt.fill" : "moon.fill",
    "--color",
    value === "Running" ? "#4C8DFF" : "#8E8E93",
  ];
  const workspaceId = firstString(process.env.CMUX_WORKSPACE_ID);
  if (workspaceId) args.push("--workspace", workspaceId);
  const result = runCmux(args, cwd);
  if (!result.ok) {
    warn(ctx, "failed to update shared cmux status", {
      value,
...
function notifyCompletion(ctx: ExtensionContext, message: string): boolean {
  ...
  if (!result.ok) {
    warn(ctx, "failed to send cmux completion notification", {
      status: result.status,
      stderr_available: result.stderr.trim().length > 0,
      error_available: result.error !== undefined,
    });
...
export default function cmuxPiSessionExtension(pi: ExtensionAPI) {
  // Child agents inherit the host's cmux surface. Only the host may own its status,
  // notifications, Feed events, or resume binding.
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (sessionId) {
      ...
      state.stopped = false;
      ensureResumeBinding(ctx, sessionId, cwd);
    }
    setSharedStatus(ctx, "Idle");
  });

  pi.on("before_agent_start", async (event, ctx) => {
    const sessionId = sessionIdFrom(ctx);
    if (sessionId) beginTurn(sessionId, event);
    setSharedStatus(ctx, "Running");
  });
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    if (!sessionId) return;
    const state = stateFor(sessionId);
    state.lastAssistantMessage = lastAssistantMessage(event) || state.lastAssistantMessage;
  });
  ...
  pi.on("agent_settled", async (event, ctx) => {
    ...
    if (!sessionId) return;
    const state = stateFor(sessionId);
    finishTurn(sessionId, event);
    notifyCompletion(ctx, state.lastAssistantMessage || "Task completed");
    setSharedStatus(ctx, "Idle");
    state.lastAssistantMessage = undefined;
  });
