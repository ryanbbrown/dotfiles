...
- Avoid metaphors and clichés: drive, unlock, deep dive, robust, key, synergy, hub, portal, landscape, ecosystem, going forward.
...
- Do not use bold or italics for emphasis. Plain words and good structure carry the meaning. Bold is only acceptable to name a literal interface element in an instruction, for example: select Save. Use double quotation marks for the titles of programs or documents, not italics.
- Use sentence case everywhere — headings, titles, table headers, all of it. Capitalize only proper nouns.
...
- Do not use Latin abbreviations. Write "for example" not "e.g.", "that is" not "i.e.", "and so on" or "such as" not "etc." They confuse screen readers and some readers.
- Ampersands: write "and", not "&" (except in a registered name or logo).
- Numbers: write "one" but use numerals for 2 and above (2, 9, 25). Use the % symbol with numerals (50%). Use $ with no decimals unless there are cents ($75, $75.50). Spell out millions and billions ($5 million, not $5M). Write ranges with "to", not a hyphen (10 to 20, Monday to Friday).
- Dates and times: write "June 4, 2026" (no "th"). Use "to" for ranges ("June 4 to 8"). Write times as "10am to 11:30am"; use "noon" and "midnight".
