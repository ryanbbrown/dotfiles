...
- Keep exactly one ordinary queue item for each unarchived direct child, and one unarchived direct child for each ordinary item. Use the child's exact 2–3 word title; do not show its thread ID.
...
- Summarize child reports and link their artifact or thread for depth. Do not paste a full report, long inventory, or large proposed file into the queue unless the user must review that exact text to decide.
