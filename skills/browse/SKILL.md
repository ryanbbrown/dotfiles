...
```bash
_UPD=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-update-check 2>/dev/null || .claude/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.gstack/sessions
touch ~/.gstack/sessions/"$PPID"
_SESSIONS=$(find ~/.gstack/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.gstack/sessions -mmin +120 -type f -exec rm {} + 2>/dev/null || true
_PROACTIVE=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.gstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${HOME}/.dotfiles/vendor/gstack/bin/gstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_SESSION_KIND=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-session-kind 2>/dev/null || echo "interactive")
...
if [ "$_ACTIVATED" = "no" ] && [ "$_SESSION_KIND" != "headless" ]; then
  _FIRST_TASK=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-first-task-detect 2>/dev/null || true)
fi
echo "FIRST_TASK: $_FIRST_TASK"
_LAKE_SEEN=$([ -f ~/.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.gstack/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "${HOME}/.dotfiles/vendor/gstack/bin/gstack-telemetry-log" ]; then
      ${HOME}/.dotfiles/vendor/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${GSTACK_HOME:-$HOME/.gstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ${HOME}/.dotfiles/vendor/gstack/bin/gstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
${HOME}/.dotfiles/vendor/gstack/bin/gstack-timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get routing_declined 2>/dev/null || echo "false")
...
_CHECKPOINT_MODE=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(${HOME}/.dotfiles/vendor/gstack/bin/gstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `${HOME}/.dotfiles/vendor/gstack/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `${HOME}/.dotfiles/vendor/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running gstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `${HOME}/.dotfiles/vendor/gstack/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `${HOME}/.dotfiles/vendor/gstack/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set explain_level terse`.
...
If A: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set telemetry anonymous`
If B→B: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set telemetry off`
...
If A: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set proactive true`
If B: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set proactive false`
...
```bash
${HOME}/.dotfiles/vendor/gstack/bin/gstack-telemetry-log --event-type first_task_scaffold_shown --skill "TASK_TOKEN" --outcome shown 2>/dev/null || true
...
If B: run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.
...
3. Run `${HOME}/.dotfiles/vendor/gstack/bin/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate gstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ${HOME}/.dotfiles/vendor/gstack && ./setup --team`"
...
_BRAIN_SYNC_BIN="${HOME}/.dotfiles/vendor/gstack/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN="${HOME}/.dotfiles/vendor/gstack/bin/gstack-config"
...
```bash
"${HOME}/.dotfiles/vendor/gstack/bin/gstack-brain-sync" --discover-new 2>/dev/null || true
"${HOME}/.dotfiles/vendor/gstack/bin/gstack-brain-sync" --once 2>/dev/null || true
```
...
```bash
${HOME}/.dotfiles/vendor/gstack/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
${HOME}/.dotfiles/vendor/gstack/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ${HOME}/.dotfiles/vendor/gstack/bin/gstack-telemetry-log ]; then
  ${HOME}/.dotfiles/vendor/gstack/bin/gstack-telemetry-log \
...
[ -z "$B" ] && B="${HOME}/.dotfiles/vendor/gstack/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/gstack/browse/dist/browse` or `${HOME}/.dotfiles/vendor/gstack/browse/dist/browse`).
