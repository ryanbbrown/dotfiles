...
Read the plan file specified by the user (usually in `.context/`). If no file is specified, ask which plan to use. Also read the corresponding spec in `.specs/` if one exists, to pull in acceptance criteria.
...
2. For each step, create a ticket using `tk create` with:
   ...
   - `--description`: a short summary of what this step accomplishes, plus a reference to the plan file and step number for full context (e.g. "See .context/plan-sessions.md, Step 3")
